...
const HighLowIndicator = ({ currentPrice, high, low }) => {
  ...
  return (
    <>
      {green && (
        <>
          <span className={`bg-red h-1.5 rounded-l-lg`} style={{ width: `${100 - green}%` }}>
            &nbsp;
          </span>
          <span className={`bg-green h-1.5 rounded-r-lg`} style={{ width: `${green}%` }}>
            &nbsp;
          </span>
        </>
      )}
...
const CryptoDetails = () => {
  const { setCoinData, currency, error, getCoinData, coinData: data } = useContext(CryptoContext);
  ...
  return ReactDOM.createPortal(
    <div
      className='fixed top-0 z-20 flex items-center justify-center w-full h-full bg-gray-200 bg-opacity-30 backdrop-blur-sm font-nunito '
      onClick={() => handleClose()}>
      <div
        ...
        {data ? (
          <div className='relative flex flex-col items-center justify-between w-full h-auto p-4 md:flex-row lg:h-full'>
            <div className='flex flex-col  md:w-[45%] w-full h-full pr-2'>
              <div className='flex items-center w-full '>
                <img src={data.image.large} alt={data.id} className='w-[3rem] h-[3rem] mx-1.5' />
              ...
              <div className='flex w-full mt-6'>
                <div className='flex flex-col w-full'>
                  <div className='flex justify-between'>
                    <span className='text-sm text-gray-100 capitalize'>Price</span>
                    <div
              ...
              <div className='flex flex-col justify-between w-full mt-4 sm:flex-row'>
                ...
                <div className='flex flex-col content-start mt-1 sm:mt-0'>
            ...
            <div className='flex flex-col justify-between md:justify-start md:w-[55%] w-full h-[60vh] md:pl-4 pl-0 md:mt-0 mt-2'>
              ...
              <div className='flex flex-col mt-4'>
                <h3 className='py-1 text-white'>
                  <span className='mr-1 text-gray-100 capitalize'>market cap rank:</span>
                  {data.market_cap_rank}
                </h3>
                <h3 className='py-1 text-white'>
                  <span className='mr-1 text-gray-100 capitalize'>coinGecko rank:</span>
                  {data.coingecko_rank}
                </h3>
                <h3 className='py-1 text-white'>
                  <span className='mr-1 text-gray-100 capitalize'>coinGecko score:</span>
                  {data.coingecko_score}
                </h3>
              </div>
            </div>
            <div className='absolute flex flex-col items-center md:bottom-8 bottom-4 right-4 md:flex-row sm:right-8'>
              {data?.links?.repos_url?.github[0] && (
                <a
                  href={data?.links?.repos_url?.github[0]}
                  target={'_blank'}
                  rel='noreferrer'
                  className='px-1 py-1 text-lg md:py-0'>
                  <svg
                    xmlns='http://www.w3.org/2000/svg'
                    width='1em'
                    height='1em'
              ...
              {data?.links?.facebook_username && (
                <a
                  ...
                  target={'_blank'}
                  rel='noreferrer'
                  className='pt-1 pl-1 text-lg md:pt-0'>
                  <svg
                    xmlns='http://www.w3.org/2000/svg'
                    width='1em'
                    height='1em'
          ...
          <div className='flex items-center justify-center w-full h-full'>
            <span className='ml-2 text-base text-red'>
              {error.coinData ? error.coinData : 'Something unexpected happened!'}
            </span>
          </div>
        ) : null}
      </div>
