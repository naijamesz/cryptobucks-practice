...
const CustomTooltip = ({ active, payload, label, currency = 'usd' }) => {
  if (active && payload && payload.length) {
    return (
      <div>
        <p className='text-sm label text-cyan'>{`${label} : ${new Intl.NumberFormat('en-US', {
          style: 'currency',
          currency: currency,
  ...
  return (
    <ResponsiveContainer height='90%'>
      <LineChart data={chartData}>
        ...
        <Tooltip cursor={false} content={<CustomTooltip />} wrapperStyle={{ outline: 'none' }} currency={currency} />
      </LineChart>
    </ResponsiveContainer>
  );
};
